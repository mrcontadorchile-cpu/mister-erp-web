...
export function NuevoPresupuestoForm({ accounts, costCenters }: Props) {
  ...
  // Paso 1: cabecera del presupuesto
  const now = new Date()
  ...
  function setAmount(key: string, monthIdx: number, value: string) {
    ...
    setLines(prev => prev.map(l =>
      l.key === key
        ? { ...l, amounts: l.amounts.map((v, i) => i === monthIdx ? n : v) }
        : l
    ))
  }
  ...
  function distributeAnnual() {
    ...
    setLines(prev => {
      ...
      if (existing) {
        return prev.map(l =>
          l.key === key ? { ...l, amounts: Array(12).fill(monthly) } : l
        )
      }
      return [...prev, {
        key,
        account_id:     annualAccountId,
        cost_center_id: annualCcId,
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      const res = await createBudget(name, fiscalYear, description, budgetLines)
          ...
          <div className="card p-6 space-y-5">
            <div>
              <label className="block text-xs font-medium text-text-secondary mb-1.5">
                Nombre del presupuesto <span className="text-error">*</span>
              </label>
              <input
                type="text"
                value={name}
            ...
            <div>
              <label className="block text-xs font-medium text-text-secondary mb-1.5">
                Año Fiscal <span className="text-error">*</span>
              </label>
              <select
                value={fiscalYear}
      ...
      {step === 2 && (
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-4 p-4 bg-primary/5 border border-primary/20 rounded-xl">
            ...
            <div>
              <p className="text-xs text-text-disabled">Año Fiscal</p>
              <p className="text-sm font-bold text-text-primary">{fiscalYear}</p>
            </div>
            <div>
              <p className="text-xs text-text-disabled">Líneas</p>
              <p className="text-sm font-bold text-text-primary">{lines.length}</p>
            </div>
              ...
              <div className="card p-4 sticky top-4">
                ...
                <div className="bg-surface-high rounded-lg p-3 mb-4 space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      type="button"
                      onClick={distributeAnnual}
                      className="btn-primary px-3 py-1.5 text-xs shrink-0"
                      title="Distribuir en 12 meses iguales"
                    >
                      /12
                    </button>
                  </div>
                  <p className="text-[10px] text-text-disabled">Divide el monto anual en 12 cuotas iguales</p>
                </div>
                ...
                <div className="space-y-1 max-h-72 overflow-y-auto pr-1">
                  {filteredAccounts.map(a => (
                    <div key={a.id} className="group">
                      ...
                      >
                        <span className={`font-mono text-[10px] shrink-0 ${ACC_TYPES[a.type]?.color ?? ''}`}>
                          {a.code}
                        </span>
                        ...
                        <div className="hidden group-hover:block pl-4 space-y-0.5 pb-1">
                          {costCenters.map(cc => (
                            <button
                              ...
                              className="w-full text-left px-2 py-1 rounded text-[10px] text-text-disabled hover:text-text-secondary hover:bg-surface-high flex items-center gap-2"
                            >
            ...
            <div className="lg:col-span-2">
              {lines.length === 0 ? (
                <div className="card p-12 text-center text-text-disabled text-sm">
                  ...
                  <p className="text-xs">Selecciona cuentas desde el panel izquierdo o usa la distribución automática</p>
                </div>
              ) : (
              ...
              {/* Footer con errores y botón */}
              {error && (
                <p className="text-xs text-error mt-3 p-3 bg-error/5 rounded-lg border border-error/20">
                  {error}
                </p>
              )}
              {lines.length > 0 && (
                <div className="mt-4 flex items-center justify-between">
                  <p className="text-xs text-text-disabled">
                    {lines.length} línea{lines.length !== 1 ? 's' : ''} · Total: <span className="text-primary font-bold">{fmtCLP(grandTotal)}</span>
                  </p>
                  <button
                    type="button"
                    onClick={handleSave}
